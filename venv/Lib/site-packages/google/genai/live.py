  ...
  def _parse_client_message(
    ...
    if isinstance(formatted_input, Sequence) and any(
      ...
      for item in contents:
        ...
        dumped_parts = content_input.model_dump(exclude_none=True, mode='json')['parts']
        for p in dumped_parts:
            # We fix function response if it's not a dict
            fr_key = 'functionResponse' if 'functionResponse' in p else 'function_response'
            if fr_key in p and not isinstance(p[fr_key], dict):
                fr = p[fr_key]
                p[fr_key] = {
